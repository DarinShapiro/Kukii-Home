...
class RunOptimizationButton(CoordinatorEntity, ButtonEntity):
    _attr_name = "SentiHome run optimization"
    _attr_unique_id = "sentihome_run_optimization"
    _attr_icon = "mdi:tune"

    async def async_press(self) -> None:
        # Wired to a real ha-agent endpoint in Epic 11.
        await self.hass.services.async_call("sentihome", "run_optimization", {})


class RetrainIdentityButton(CoordinatorEntity, ButtonEntity):
    _attr_name = "SentiHome retrain identity"
    _attr_unique_id = "sentihome_retrain_identity"
    _attr_icon = "mdi:account-arrow-up"

    async def async_press(self) -> None:
        await self.hass.services.async_call("sentihome", "label_person", {})
