...
* User-initiated (``async_step_user``): the classic "Settings → Add
  integration → SentiHome" path. Manual host/port entry.
* Zeroconf-discovered (``async_step_zeroconf``): the add-on
  broadcasts ``_sentihome._tcp.local.`` on the LAN; HA picks it up
  automatically; the user sees a "Discovered: SentiHome" card and
...
class SentiHomeConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_zeroconf(
        ...
        ``_sentihome._tcp.local.`` broadcast.
    ...
    async def async_step_zeroconf_confirm(
        ...
        if user_input is not None:
            return self.async_create_entry(
                title=f"SentiHome ({self._discovered_host}:{self._discovered_port})",
                data={
