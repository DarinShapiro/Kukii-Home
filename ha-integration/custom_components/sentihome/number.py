...
These are stubs in v1 — wiring them back to live SentiHome config is the
...
class ConfidenceThresholdNumber(CoordinatorEntity, NumberEntity):
    _attr_name = "SentiHome global confidence threshold"
    _attr_unique_id = "sentihome_confidence_threshold"
    ...
    def __init__(self, coordinator: SentiHomeCoordinator) -> None:
    ...
    async def async_set_native_value(self, value: float) -> None:
        self._value = value
        _LOGGER.info("sentihome: confidence threshold set to %.2f", value)
