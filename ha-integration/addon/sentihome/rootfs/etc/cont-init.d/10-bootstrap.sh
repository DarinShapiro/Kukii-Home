...
# - Hands Supervisor's /data/options.json to the SentiHome topology loader
#   by exposing it via SENTIHOME_CONFIG.
...
export SENTIHOME_CONFIG="$OPTIONS_FILE"
...
echo "$SENTIHOME_CONFIG" > /var/run/s6/container_environment/SENTIHOME_CONFIG

echo "[bootstrap] SentiHome topology config: $SENTIHOME_CONFIG (log_level=$LOG_LEVEL)"
