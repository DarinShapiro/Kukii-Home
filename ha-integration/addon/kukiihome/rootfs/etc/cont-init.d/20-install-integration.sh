...
# Epic 10.8.4: bundle the Kukii-Home custom integration with the add-on
# and auto-install it to /config/custom_components/kukiihome/ on every
...
SRC=/app/ha-integration/custom_components/kukiihome
DST=/config/custom_components/kukiihome
STAMP=/config/.kukiihome_integration_version
...
if [ -n "${SUPERVISOR_TOKEN:-}" ]; then
    NOTIFY_MSG="Kukii-Home companion integration was updated. Restart Home Assistant to load it: Settings → System → Power → Restart Home Assistant. Tap-to-open-alert and FP feedback won't work until you restart."
    if curl --silent --show-error --fail \
            -X POST \
            -H "Authorization: Bearer $SUPERVISOR_TOKEN" \
            -H "Content-Type: application/json" \
            -d "{\"message\":\"$NOTIFY_MSG\",\"title\":\"Kukii-Home: restart needed\",\"notification_id\":\"kukiihome_restart_required\"}" \
