# Kukii-Home add-on configuration

The Supervisor add-on UI shows a top-level form mapped to the Kukii-Home
...
| `preprocessor_url` | Epic 10.9. URL of the Kukii-Home **preprocessor** (inference box) on your LAN, e.g. `http://192.168.68.50:8090`. When set, each alert is enriched with the preprocessor's recognition — who/what was seen + an annotated frame — folded into the per-alert page. Leave blank if you don't run a separate preprocessor; alerts still fire with the HA snapshot + rule that fired. | _empty_             |
...
camera + timestamp), a link to the Kukii-Home status page, and the
snapshot image (HA Companion app renders it inline).

Selection is persisted at `/data/kukiihome/notify_overrides.json` —
...
Overrides are persisted at `/data/kukiihome/adapter_overrides.json`
...
Kukii-Home subscribes to the listed motion / AI sensors, snapshots via
...
Kukii-Home pulls the RTSP stream itself, runs MOG2 motion detection. Use
...
See `infrastructure/docker/kukiihome.example.yaml` in the repo for a
fully-annotated starter.

## Where data lives

- `/data/options.json` — Supervisor-managed; do not edit by hand
- `/data/kukiihome/` — Postgres + Qdrant + Redis volumes and the object store
- `/share/kukiihome/` — exported clips, daily digests

## Logs

Supervisor → Kukii-Home → **Log** tab. Each underlying service logs via
...
- **HA entities don't appear:** install the Kukii-Home custom integration
