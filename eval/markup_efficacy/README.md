...
Rex, Bob's truck) actually _improve_ its reasoning compared to handing
...
because we always have _some_ JSON to pass — we'd never deploy
...
- **Annotated ≈ Raw**: simplify. Stop rendering annotated JPEGs; pass
  ...
  cache storage. _Object detection (YOLO) still runs — for gating /
