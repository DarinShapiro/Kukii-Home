...
| Status                       | Count             |
| ---------------------------- | ----------------- |
| Epics closed                 | 11 of 18 (61%)    |
| Sub-issues closed            | 176 of 286 (~62%) |
...
| ---- | ---------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| #157 | Identity & Recognition       | #158-#174  | **Start here.** Multi-modal identity, multi-camera fusion, stereo verification, temporal evidence accumulation, retroactive re-eval. |
...
**Suggested epic order:** 5 → 6 → 7 → 8 → 8.5 → **8.6 → 9** (both closed; repo is HA-installable) → 10 → 11 → 12 → 13 → 14 → 15 → 16
...
│   ├── ha-agent/            WS+REST client + MCP tools + area resolver + HTTP API  (REAL)
│   └── notify/              Push/TTS/Ask dispatchers + NotifyWorker  (REAL)
│
...
| 9 (HA Integration)    | ha-agent client (REST + cache) + AreaRegistry (explicit / area_id / heuristic) + HATools (illuminate/darken/list_caps/get_changes/lock/unlock) + HTTP API (snapshot/service/ack) + AlertLog + make_ha_caller | 23 unit                          |
| **Total**             |                                                                                                                                                                                                              | **330 unit + integration suite** |
