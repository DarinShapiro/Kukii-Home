...
The pluggable NVR adapter layer that makes Kukii-Home work with any frame source — or none at all. V1 ships service-mode adapters for the major platforms plus direct RTSP for users without an NVR. Native modes are explicitly out of v1 scope.
...
11. **feat(adapter-frigate): map Frigate YOLO detections to Kukii-Home enrichment schema** — translate Frigate's metadata format. (labels: `epic:nvr-adapters`, `component:adapter-frigate`, `priority:p0`)
