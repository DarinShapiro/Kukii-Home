...
At bootstrap, Kukii-Home's core inspects environment + config to determine which
...
if TYPE_CHECKING:
    ...
    from kukiihome_shared.adapter.base import CameraCapability
    from kukiihome_shared.topology import AdapterConfig
...
def bootstrap_from_env() -> AdapterRegistry:
    ...
    Recognized env vars:
        KUKIIHOME_ADAPTER_AGENT_DVR_URL — enable AgentDVRAdapter
        KUKIIHOME_ADAPTER_FRIGATE_URL  — enable FrigateAdapter
        KUKIIHOME_ADAPTER_BLUEIRIS_URL — enable BlueIrisAdapter
        KUKIIHOME_ADAPTER_RTSP_DIRECT_CONFIG — JSON-encoded camera list
    ...
    if url := os.environ.get("KUKIIHOME_ADAPTER_AGENT_DVR_URL"):
        ...
        registry.register(
            AgentDVRAdapter(
                AgentDVRConfig(
                    base_url=url,
                    username=os.environ.get("KUKIIHOME_ADAPTER_AGENT_DVR_USERNAME"),
                    password=os.environ.get("KUKIIHOME_ADAPTER_AGENT_DVR_PASSWORD"),
                )
            )
        )

    if url := os.environ.get("KUKIIHOME_ADAPTER_FRIGATE_URL"):
        ...
        registry.register(
            FrigateAdapter(
                FrigateConfig(
                    rest_url=url,
                    mqtt_host=os.environ.get("KUKIIHOME_ADAPTER_FRIGATE_MQTT_HOST", "localhost"),
                )
            )
        )

    if rtsp_config := os.environ.get("KUKIIHOME_ADAPTER_RTSP_DIRECT_CONFIG"):
        import json
