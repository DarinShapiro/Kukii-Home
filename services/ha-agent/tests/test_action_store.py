...
def test_perception_upsert_then_read(store):
    e = PerceptionEntry(
        ...
        target="light.turn_on:light.porch", max_duration_s=120,
    )
...
def test_perception_upsert_overwrites(store):
    e = PerceptionEntry(
        camera_id="front", target_kind="camera_api",
        target="ptz_zoom", max_duration_s=30,
    )
    store.upsert_perception(e)
    # Same primary key (camera, kind, target) → update existing row
    store.upsert_perception(
        PerceptionEntry(camera_id="front", target_kind="camera_api",
                        target="ptz_zoom", max_duration_s=60)
    )
    out = store.perception_for("front")
    assert len(out) == 1
...
def test_perception_disabled_excluded_from_read(store):
    store.upsert_perception(PerceptionEntry(
        camera_id="front", target_kind="camera_api", target="ptz_zoom",
        enabled=False,
    ))
...
def test_perception_delete(store):
    store.upsert_perception(PerceptionEntry(
        camera_id="front", target_kind="ha_service", target="x",
    ))
...
def _protective(**kw):
    base: dict = dict(  # noqa: C408
        ...
        target="lock.back_door", min_severity="critical", min_confidence=0.8,
    )
    base.update(kw)
...
def test_protective_blackout_roundtrip_as_json(store):
    e = _protective(blackout_windows=[
...
def test_protective_find_exact_match(store):
    ...
    hit = store.find_protective(
        camera_id="backyard", service="lock.lock",
        target="lock.back_door", action_class="lock",
    )
    assert hit is not None
    miss = store.find_protective(
        camera_id="backyard", service="lock.lock",
        target="lock.front_door", action_class="lock",
    )
    assert miss is None
...
def test_gate_rejects_unscoped(store):
    decision = gate_recommendation(
        store=store, camera_id=None, action_class="lock",
        service="lock.lock", target="lock.x", severity="critical",
        confidence=0.99,
    )
...
def test_gate_rejects_missing_whitelist(store):
    decision = gate_recommendation(
        store=store, camera_id="any", action_class="lock",
        service="lock.lock", target="lock.x", severity="critical",
        confidence=0.99,
    )
    assert decision.execute is False
...
def test_gate_passes_when_thresholds_met(store):
    store.upsert_protective(_protective())
    decision = gate_recommendation(
        store=store, camera_id="backyard", action_class="lock",
        service="lock.lock", target="lock.back_door",
...
def test_gate_blocks_in_blackout_window(store):
    ...
    store.upsert_protective(_protective(blackout_windows=[
        {"days": ["mon"], "start": "09:00", "end": "17:00"},
    ]))
    decision = gate_recommendation(
        store=store, camera_id="backyard", action_class="lock",
        service="lock.lock", target="lock.back_door",
        severity="critical", confidence=0.95, now_ts=when,
    )
    assert decision.execute is False
...
def test_log_protective_assigns_id(store):
    row = ProtectiveLogRow(
        incident_id="i1", camera_id="backyard", ts=100.0,
...
def test_log_for_incident_returns_chrono_order(store):
    for i, ts in enumerate([200, 100, 300]):
        store.log_protective(ProtectiveLogRow(
            incident_id="inc99", camera_id="c", ts=ts,
            action_class="lock", service="lock.lock",
            target=f"lock.t{i}", data_json=None, status="ok",
        ))
...
def test_recent_log_newest_first(store):
    for ts in [100.0, 200.0, 300.0]:
        store.log_protective(ProtectiveLogRow(
            incident_id="i", camera_id="c", ts=ts,
            action_class="lock", service="lock.lock", target="x",
            data_json=None, status="ok",
        ))
...
def test_persist_to_disk_survives_reopen(tmp_path):
    ...
    s1.upsert_perception(PerceptionEntry(
        camera_id="front", target_kind="ha_service", target="x",
    ))
