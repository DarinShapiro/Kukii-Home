...
def test_list_renders_review_and_enrolled_tabs_with_counts():
    subs = [IdentitySubject(subject_id="bob", kind="person", display_name="Bob")]
...
def test_list_subjects_sorted_people_then_pets_then_vehicles():
    subs = [
        IdentitySubject(subject_id="car1", kind="vehicle", display_name="Tesla"),
        IdentitySubject(subject_id="winston", kind="pet", display_name="Winston"),
        IdentitySubject(subject_id="alice", kind="person", display_name="Alice"),
        IdentitySubject(subject_id="bob", kind="person", display_name="Bob"),
    ]
...
def test_list_html_escapes_subject_names():
    html = render_identities_list(
        [IdentitySubject(subject_id="x", kind="person", display_name="<script>")],
    )
...
def test_list_subject_tile_links_to_detail():
    subs = [IdentitySubject(subject_id="bob_42", kind="person", display_name="Bob")]
    html = render_identities_list(subs)
...
def test_detail_no_modalities_shows_empty_marker():
    s = IdentitySubject(subject_id="x", kind="person", display_name="X", modalities=[])
    ...
    vm = _vm(
        linked_guidance=[
            GuidanceEntry(
                ...
                name="Bob arrives",
                storage_class="rule",
                scope_summary="front_door",
                detail_url="memory?cut=by_type#r1",
            ),
            GuidanceEntry(
                guidance_id="p1",
                name="Watch for Bob tonight",
                storage_class="transient_intent",
                scope_summary="tonight",
                detail_url="memory#p1",
            ),
        ]
    )
...
def test_detail_includes_kind_icon_for_pets():
    s = IdentitySubject(subject_id="winston", kind="pet", display_name="Winston", species="dog")
    html = render_identity_detail(IdentityDetailViewModel(subject=s))
...
def test_filter_matches_by_actor_name():
    bob = IdentitySubject(subject_id="bob_42", kind="person", display_name="Bob")
