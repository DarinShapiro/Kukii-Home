...
def test_matched_rules_section_lists_each_rule():
    matches = [
        RuleMatch(rule_id="winston_outdoors", incident_id="i1",
                   matched_at=NOW, severity="critical", confidence=0.92,
                   reasoning="pet alone in front", matched=True),
        RuleMatch(rule_id="bob_arrives", incident_id="i1",
                   matched_at=NOW, severity="normal", confidence=0.88,
                   reasoning="bob recognized", matched=True),
    ]
...
def test_matched_rules_section_marks_non_matches():
    matches = [
        RuleMatch(rule_id="delivery", incident_id="i", matched_at=NOW,
                   severity=None, confidence=0.4, reasoning="ambiguous",
                   matched=False),
    ]
    html = render_matched_rules_section(matches)
...
def _log(status, *, gate="", action_class="lock", ts=NOW):
    return ProtectiveLogRow(
        incident_id="i", camera_id="back", ts=ts,
        action_class=action_class, service="lock.lock", target="lock.back_door",
        data_json=None, status=status, gate_reason=gate or None,
    )
...
def test_policy_hits_section_resolves_policy_names():
    ...
    hits = [
        PolicyHit(policy_id="p1", incident_id="i", applied_at=NOW,
                   outcome="dismissed"),
    ]
...
def test_policy_hits_section_unknown_policy_falls_back_to_id():
    hits = [PolicyHit(policy_id="unknown_pol", incident_id="i",
...
def test_build_audit_chain_empty_when_stores_present_but_no_hits(stores):
    ...
    html = build_audit_chain_html(
        incident_id="ghost",
        rules_store=r, action_store=a, policy_store=p, now_ts=NOW,
    )
    assert html == ""
...
def test_build_audit_chain_assembles_all_three_sections(stores):
    ...
    r.record_match(RuleMatch(rule_id=rule.id, incident_id="inc1",
                              matched_at=NOW, severity="critical",
                              confidence=0.9, reasoning="x", matched=True))
    a.log_protective(ProtectiveLogRow(
        incident_id="inc1", camera_id="back", ts=NOW,
    ...
    p.record_hit(PolicyHit(policy_id=pol.id, incident_id="inc1",
                            applied_at=NOW, outcome="dismissed"))
...
def test_build_audit_chain_partial_sections_when_only_some_stores_have_hits(stores):
    ...
    r.record_match(RuleMatch(rule_id=rule.id, incident_id="inc",
                              matched_at=NOW, severity="normal",
                              confidence=0.7, reasoning="", matched=True))
...
def test_build_audit_chain_tolerates_store_exceptions():
    ...
    html = build_audit_chain_html(
        incident_id="i", rules_store=_BoomStore(),
        action_store=_BoomStore(), policy_store=_BoomStore(), now_ts=NOW,
    )
