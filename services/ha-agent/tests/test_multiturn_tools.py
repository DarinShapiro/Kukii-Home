...
def test_prompt_includes_last_committed_summary():
    ctx = _ctx(last_committed=CommittedEntrySummary(
...
async def test_search_tool_finds_matching_rule(stores):
    ...
    r.create(Rule(
        id="", name="Winston alone in Front yard", mode="nl",
        intent_text="Winston in Front yard alone — alert critical.",
        scope=RuleScope(areas=["front_yard"]),
    ))
    tool = SearchExistingGuidance(rules_store=r, policy_store=p,
                                    preferences_store=pr)
...
async def test_search_tool_caps_matches_at_ten(stores):
    ...
    from kukiihome_ha_agent.rules_store import RuleScope
    for i in range(15):
        r.create(Rule(id="", name=f"R{i}", mode="nl",
                       intent_text="x",
                       scope=RuleScope(areas=["x_area"])))
    tool = SearchExistingGuidance(rules_store=r, policy_store=p,
                                    preferences_store=pr)
...
async def test_get_known_actor_finds_subject_by_display_name():
    client = _FakePreprocClient([
        {"subject_id": "winston", "kind": "pet", "display_name": "Winston",
         "species": "dog", "modalities": ["pet", "gait"], "appearances": 12},
    ])
...
async def test_tool_loop_executes_tool_then_returns_proposal(stores):
    ...
    the proposal."""
    r, p, pr = stores
    tools = [SearchExistingGuidance(rules_store=r, policy_store=p,
                                      preferences_store=pr)]
...
async def test_tool_loop_exhausts_rounds_and_raises():
    ...
    forever_tool_response = {
        "content": "",
        "tool_calls": [{
            "id": "x", "function": {"name": "foo", "arguments": "{}"},
        }],
    }
...
def commit_stores():
    ...
    yield bundle
    for s in (bundle.rules, bundle.preferences, bundle.policies,
              bundle.provenance):
        if s:
            s.close()
...
def test_commit_routes_refines_guidance_id_to_refine(commit_stores):
    ...
    p_initial = PlacementProposal(
        ...
        reasoning="initial placement", confidence=0.9,
    )
    gid = commit_guidance(p_initial, stores=commit_stores,
                          origin="conversation",
                          transcript_id="t0", user_utterance="initial")
    ...
    p_refined = PlacementProposal(
        ...
        reasoning="refined scope", confidence=0.95,
    )
    gid2 = commit_guidance(p_refined, stores=commit_stores,
                            origin="conversation",
                            transcript_id="t1", user_utterance="only at night")
    # Same id returned
    assert gid2 == gid
...
def test_commit_with_unknown_refines_id_falls_through_to_fresh_create(commit_stores):
    ...
    p = PlacementProposal(
        storage_class="rule", name="Test",
        scope={"refines_guidance_id": "rule_ghost"},
        lifecycle="persistent", fire_affordance="alert",
        intent_text="test", reasoning="r", confidence=0.9,
    )
