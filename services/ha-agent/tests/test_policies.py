...
def test_all_policies_hides_expired_ttl_even_without_revoke(store):
    ...
    assert all(x.id != p.id for x in store.all_policies(now_ts=NOW, kind="dismissal"))


def test_all_policies_keeps_unexpired_ttl(store):
    p = store.create(_dismissal(expires_at=NOW + 3600))
    assert any(x.id == p.id for x in store.all_policies(now_ts=NOW, kind="dismissal"))
...
def test_hits_for_incident_reverse_link(store):
    ...
    store.record_hit(
        PolicyHit(policy_id=p1.id, incident_id="inc99", applied_at=NOW, outcome="dismissed")
    )
    store.record_hit(
        PolicyHit(policy_id=p2.id, incident_id="inc99", applied_at=NOW, outcome="dismissed")
    )
...
def test_render_policies_page_lists_with_revoke_buttons():
    pols = [
        Policy(
            ...
            rationale="user ✗-ed",
            apply_count=3,
            last_applied_at=NOW - 600,
            created_at=NOW - 86400,
        ),
        Policy(id="p2", kind="dismissal", name="Wind in tree", apply_count=0, created_at=NOW - 100),
    ]
    html = render_policies_page(
        dismissals=pols,
        transient_intents=[],
...
def test_render_policies_page_transient_intents_section_separate():
    intent = Policy(
        id="ti1", kind="transient_intent", name="Watch for Bob", apply_count=0, created_at=NOW
    )
    html = render_policies_page(
        dismissals=[],
        transient_intents=[intent],
        now_ts=NOW,
    )
...
def test_render_policies_page_html_escapes_policy_names():
    p = Policy(id="x", kind="dismissal", name="<script>", created_at=NOW)
...
def test_render_policies_page_expires_line_present_when_ttl_set():
    p = Policy(id="p", kind="dismissal", name="TTL'd", expires_at=NOW + 3600, created_at=NOW)
    html = render_policies_page(dismissals=[p], transient_intents=[], now_ts=NOW)
