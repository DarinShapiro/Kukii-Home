...
class HAAgentSettings:
    ...
    def from_topology(cls, topology: Any) -> HAAgentSettings:
        """Build from :class:`sentihome_shared.topology.HAAgentConfig`."""
        cfg = topology.ha_agent
        if not cfg.ha_token:
            raise ValueError(
                "ha_agent.ha_token is empty — set it in sentihome.yaml or via "
                "SENTIHOME__HA_AGENT__HA_TOKEN env var (Supervisor add-on "
