...
The tool surface Kukii-Home services call to read + write HA. Modeled after
...
``services/core/dispatch.py`` so Kukii-Home's autonomous-action policy is
...
# Domains we surface as "capabilities" so Kukii-Home knows what's hooked up.
CAPABILITY_DOMAINS = (
    "light",
    "switch",
    "lock",
...
class HACameraEntity:
    """An HA camera the user could point a Kukii-Home adapter at."""
...
class HATools:
    """The MCP-shaped surface Kukii-Home services interact with."""
    ...
    async def call_service(
        ...
        *,
        ...
        (see :class:`kukiihome_core.dispatch.PolicyGate`)."""
