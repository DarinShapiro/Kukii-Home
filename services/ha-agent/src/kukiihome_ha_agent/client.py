...
The client is the single low-level seam between Kukii-Home and HA Core:
...
class HAClient:
    ...
    async def fetch_camera_snapshot(self, entity_id: str) -> bytes:
        ...
        use Kukii-Home's ``rtsp-direct`` adapter with the camera's RTSP
        URL + credentials so Kukii-Home bypasses HA's image-fetch entirely.
        """
    ...
    async def sign_url(self, path: str) -> str | None:
        """Ask the Kukii-Home integration to sign a /api/kukiihome/...
        ...
        """
        try:
            resp = await self._http.get("/api/kukiihome/sign", params={"path": path})
