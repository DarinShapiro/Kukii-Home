"""Zeroconf / mDNS publisher so HA auto-discovers the Kukii-Home integration.

Epic 10.8.4: removes the "type the host + port into the integration
config flow" step. The add-on publishes a ``_kukiihome._tcp.local.``
service on the local network; HA's zeroconf component sees it; the
Kukii-Home custom integration's manifest declares it picks up that
service; HA shows "Discovered: Kukii-Home" in Devices & Services with
a single Configure button.

The service payload carries:

* ``host`` + ``port`` — where ha-agent's HTTP API listens. The
  integration uses these to construct its Kukii-HomeAPIClient base URL.
...
# Standard mDNS service type for Kukii-Home. Picked deliberately — no
# existing service uses this; the integration's manifest pins this
# exact string.
_SERVICE_TYPE = "_kukiihome._tcp.local."
...
def publish_kukiihome(
    *,
    ...
    hostname = socket.gethostname() or "kukiihome"
    ...
    info = ServiceInfo(
        type_=_SERVICE_TYPE,
        name=f"Kukii-Home ({hostname})._kukiihome._tcp.local.",
