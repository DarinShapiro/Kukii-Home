...
Payload shape — what each notify service sees:
  ...
  - ``data.url`` — deep-link to the tapped alert: the HA panel route
    ...
    Tapping opens the Kukii-Home panel in-app (authenticated), and the
    panel's in-panel reader navigates to that alert's detail page.
  - ``data.image`` — link to the alert's snapshot (when one was
    captured). The HA Companion app fetches this via the Kukii-Home
...
class AlertNotifier:
    ...
    kukiihome_ingress_base: str = ""
    """Optional base URL for the Kukii-Home ingress prefix
    ...
    async def send(self, alert: dict[str, Any]) -> None:
        ...
        Used by :class:`~kukiihome_ha_agent.triage.TriageGate` once
    ...
    def _render(self, alert: dict[str, Any]) -> tuple[str, str, dict[str, Any]]:
        ...
        URLs are computed against ``kukiihome_ingress_base`` when set
        so they route through HA Ingress (visible to the user as the
        Kukii-Home Web UI, with HA Companion's auth). When the prefix
        ...
        if camera_id:
            data["tag"] = f"kukiihome_{camera_id}"

        # Tap-action URL: the Kukii-Home HA frontend panel route
        ...
        # /api/kukiihome/alert/<id> signed + unsigned (401: the HA
        ...
        #
        # /app/<slug> opens the Kukii-Home panel in-app with the
        ...
        # v0.3.18 — high-priority delivery flags. Kukii-Home's portion
