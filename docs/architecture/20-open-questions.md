...
| 2026-06-02 | **The principle extends across servers — shape the pipeline to whatever hardware exists; ingestion is the cleanest partition axis** | A fourth (host-level) parallelism axis. The pipeline is a graph of independent stages over NATS, so every edge is a potential partition line; distribution is a deployment choice, not a code change. Three partition points already exist in code: **ingestion** (per-camera tasks + `node_id` + `preprocessor_node_id`; split cameras across boxes via the CAMERAS list — no code change, ~zero transport cost since a camera's frames stay local through detect+crop and only the embedding crosses the wire at correlation), **branch** (`placement_hint` + RemotePipeline 10.11.4, ship the crop — pays transport, worth it only when compute ≫ transport), and **stage** (NATS subjects between services). A second _high-capacity_ peer upgrades the secondary-server role from weak failover to a true peer partitioned by workload, both saturated continuously. Placement decided by the DAG simulator (already models multi-host network latency); ResourcePool generalizes to per-host named pools. | §10.11, §10.12, §08, §08.5    | Adopted (design in 10.12)        |
