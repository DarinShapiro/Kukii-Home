...
class FrameWindow(_Strict):
    ...
    :class:`kukiihome_shared.timing.StepTimings`. Empty for
...
class CameraConfigEvent(_Strict):
    ...
    * HA HLS proxy (``http://homeassistant:8123/api/hls/.../playlist.m3u8``)
    ...
    camera_id: str
    """Stable identifier across the Kukii-Home topology. Matches the
...
class ActorEnrollmentEvent(_Strict):
    ...
    Preprocessor subscribes to the three ``kukiihome.memory.actor.*``
