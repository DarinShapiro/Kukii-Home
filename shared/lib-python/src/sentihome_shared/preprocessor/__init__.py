...
* :mod:`sentihome_shared.preprocessor.contracts` — Pydantic schemas
  for every payload that crosses the process boundary.
* :mod:`sentihome_shared.preprocessor.nats_subjects` — canonical NATS
  topic names for the (small) NATS surface — broadcast-only config
  state from memory to preprocessor.
* :mod:`sentihome_shared.preprocessor.client` — thin httpx-based
