"""Smoke tests for the kukiihome_shared library modules."""
...
def test_tracing_new_ids() -> None:
    from kukiihome_shared.tracing import new_span_id, new_trace_id
...
def test_config_load_from_env(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    monkeypatch.setenv("KUKIIHOME_TEST_NATS_URL", "nats://example:4222")
    monkeypatch.setenv("KUKIIHOME_TEST_LOG_LEVEL", "DEBUG")

    cfg = load_config(CoreConfig, prefix="KUKIIHOME_TEST_")
    assert cfg.nats_url == "nats://example:4222"
    assert cfg.log_level == "DEBUG"


def test_mcp_policy_gate_error_payload() -> None:
    from kukiihome_shared.mcp import PolicyGateError
...
def test_bus_module_imports() -> None:
    ...
    from kukiihome_shared import bus
